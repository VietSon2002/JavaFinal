...
public class UserImpl implements UserDao {
    ...
    public User findUser(String email,String password){
        try {
            String sql;
            if(email.contains("@"))sql="select * from users where email='"+email+"' and password='"+API.getMd5(password)+"'";
            else sql="select * from users where phone='"+email+"' and password='"+API.getMd5(password)+"'";
